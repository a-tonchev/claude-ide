...
const OBSERVER_INSTRUCTIONS = [
  ...
  '',
  '## MCP Tools',
  'You have two extra MCP tools in addition to the standard ones:',
  ...
  '2. If no instructions exist (empty):',
  '   - Ask the user for SSH credentials and task description (use user_input_needed MCP tool)',
  '   - Save them to instructions via setObserver IMMEDIATELY before doing anything else',
  '3. If instructions exist, resume from where you left off',
  '',
  ...
  '- Always include: SSH credentials, working directory, workspace path, tmux session name, approval policy, permissions approach, current status, progress summary, last processed timestamp',
